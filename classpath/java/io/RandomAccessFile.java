...
public class RandomAccessFile {
    ...
    throws FileNotFoundException
  {
    ...
    if (mode.equals("rw")) readWrite = true;
  ...
  private void open() throws FileNotFoundException {
    ...
    open(file.getPath(), readWrite, result);
    peer = result[0];
    length = result[1];
  }

  private static native void open(String name, boolean readWrite, long[] result)
    throws FileNotFoundException;
  ...
  public void seek(long position) throws IOException {
    if (position < 0 || (!readWrite && position > length())) throw new IOException();
